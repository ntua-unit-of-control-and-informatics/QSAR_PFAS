...
rng = RandomState(100)  # good is 1  # Create a random number generator with seed 42
for i in range(0, len(all_smiles), 10):
    batch = all_smiles[i : i + 10]
    if len(batch) >= 3:
        batch_test = rng.choice(batch, size=3, replace=False)
    else:
        batch_test = rng.choice(batch, size=max(1, len(batch) // 3), replace=False)
    test_smiles.extend(batch_test)
    test_smiles.extend(batch_test)

# Create test dataset
test_df = halflife_df[halflife_df["SMILES"].isin(test_smiles)]
print(np.unique(test_df["PFAS"].values))
train_df = halflife_df[~halflife_df["SMILES"].isin(test_smiles)]
